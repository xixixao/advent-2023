const input = `Game 1: 7 blue, 4 red, 11 green; 2 red, 2 blue, 7 green; 2 red, 13 blue, 8 green; 18 blue, 7 green, 5 red
...
const games = input.split("\n").map((rawGameWithName) => {
  const [, rawGame] = rawGameWithName.split(": ");
  const rawSets = rawGame.split("; ");

  return rawSets.map((rawNumbers) => {
    const numbers = {
      red: 0,
      blue: 0,
      green: 0,
    };
    rawNumbers.split(", ").forEach((rawNumber) => {
      const [number, color] = rawNumber.split(" ");
...
const gamesMaxPerColor = games.map((sets) => {
  const maxPerColor = {
    red: 0,
    blue: 0,
    green: 0,
  ...
  sets.forEach((numbers) => {
    Object.keys(numbers).forEach((color) => {
      maxPerColor[color] = Math.max(maxPerColor[color], numbers[color]);
    });
  });
  return maxPerColor;
});
...
const gamesIsPossible = gamesMaxPerColor.map(
  (max) => max["red"] <= 12 && max["green"] <= 13 && max["blue"] <= 14
);

const result = gamesIsPossible
