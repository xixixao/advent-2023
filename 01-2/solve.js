...
const linesTransformedForward = lines.map((line) =>
  line.replace(
...
const linesTransformedBackward = lines.map((line) =>
  reverseString(line).replace(
    /eno|owt|eerht|ruof|evif|xis|neves|thgie|enin/g,
    (match) => numbers[reverseString(match)]
  )
);

const linesFirstDigit = linesTransformedForward.map((line) =>
  line
    .split("")
...
const linesLastDigit = linesTransformedBackward.map((line) =>
  line
    .split("")
    .map((char) => parseInt(char))
    .filter((digit) => !isNaN(digit))
...
const linesCombinedDigits = linesFirstDigit.map(
  (digit, index) => digit + "" + linesLastDigit[index]
);

const linesNumbers = linesCombinedDigits.map((digits) => parseInt(digits));

const result = linesNumbers.reduce((acc, curr) => acc + curr, 0);
