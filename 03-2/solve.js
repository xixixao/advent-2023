...
const gearsToNumbers = [];

const lineLength = lines[0].length;
const linesCount = lines.length;

// let debug = lines.map((line) => line.split(""));
...
for (let y = 0; y < linesCount; y++) {
  for (let x = 0; x < lineLength; x++) {
    ...
    const number = parseInt(lines[y].slice(x));
    if (isNaN(number)) {
      continue;
    }
    const string = number.toString();

    /// check all adjecent cells for gears
    // let found = false;
    for (
      let yl = Math.max(y - 1, 0);
      yl <= Math.min(y + 1, linesCount - 1);
      yl++
    ) {
      for (
        let xl = Math.max(x - 1, 0);
        xl <= Math.min(x + string.length, lineLength - 1);
        xl++
      ) {
        if (lines[yl][xl] === "*") {
          ((gearsToNumbers[yl] ??= [])[xl] ??= []).push(number);
        }
      }
    }
    // if (!found) {
...
// console.log(debug.map((line) => line.join("")).join("\n"));

const validGears = gearsToNumbers
  .flat()
  .filter((numbers) => numbers.length === 2);

const ratios = validGears.map(([a, b]) => a * b);

const result = ratios.reduce((a, b) => a + b, 0);
